...
async function seedDashboard(page) {
  await page.evaluate(() => {
    ...
    child.skills = [
      { id: 'skill-a', name: 'Giao tiếp', level: 8, date: today },
      { id: 'skill-b', name: 'Tư duy logic', level: 7, date: today }
    ];
    child.healthRecords = [{ id: 'health-a', date: today, height: 118, weight: 22, sleep: 10, note: '' }];
    child.nutritionLogs = [{ id: 'nutrition-a', date: today, water: 5, plants: true, protein: true, note: '' }];
    child.physicalActivities = [{ id: 'physical-a', activity: 'Chơi ngoài trời', date: today, minutes: 30 }];
    child.habits = [
      { id: 'habit-a', title: 'Thức dậy đúng giờ', frequency: 'Hằng ngày', logs: [today] },
...
test('premium overview uses real local child data and keeps navigation functional', async ({ page }) => {
  ...
  const sourceOverview = page.locator('.premium-source-overview');
  await expect(sourceOverview).toHaveCount(3);
  expect(await sourceOverview.evaluateAll((nodes) => nodes.every((node) => getComputedStyle(node).display === 'none'))).toBe(true);
