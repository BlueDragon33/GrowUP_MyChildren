...
test('new learning record stores domain metadata and returns to learning page after synchronized reload',async({page})=>{
  ...
  await expect(page.locator('.topbar h1')).toHaveText('Học tập');
  await expect(page.locator('.item-title').filter({hasText:'Taxonomy reading goal'})).toBeVisible();
  ...
  expect(await page.evaluate(()=>sessionStorage.getItem('growup_v9_resume_nav'))).toBeNull();
});
