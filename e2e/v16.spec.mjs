...
async function child(page){await page.goto('/');await page.locator('#emptyAddChild').click();await page.locator('#childForm input[name="name"]').fill('V16 Child');await page.locator('#childForm input[name="dateOfBirth"]').fill('2018-01-01');await page.locator('#childForm button.primary').click();await expect(page.locator('[data-v16="receipt-integrity"]')).toBeVisible();await expect(page.locator('[data-v17="receipt-history"]')).toBeVisible();}
async function reload(page){await page.reload();await expect(page.locator('[data-v16="receipt-integrity"]')).toBeVisible();await expect(page.locator('[data-v17="receipt-history"]')).toBeVisible();}

test('v16 integrity remains enforced while managed v17 path blocks legacy import bypass',async({page})=>{
  await child(page);
  await expect(page.locator('#v16ReceiptIntegrityImportForm')).toBeHidden();
  await expect(page.locator('#v16PresetIntegrityImportForm')).toBeHidden();
  ...
  await page.locator('#v17ReceiptImportForm input[name="file"]').setInputFiles({name:'bad.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(bad))});await page.locator('#v17ReceiptImportForm button[type="submit"]').click();await expect(page.locator('#v17ApplyReceipt')).toBeDisabled();
  await page.locator('#v17ReceiptImportForm input[name="file"]').setInputFiles({name:'ok.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(receiptPkg))});await page.locator('#v17ReceiptImportForm button[type="submit"]').click();await expect(page.locator('#v17ApplyReceipt')).toBeEnabled();
  ...
  const presetPkg=buildIntegrityWorkloadPresetPackage(presetSettings);await page.locator('#v17PresetImportForm input[name="file"]').setInputFiles({name:'preset.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(presetPkg))});await page.locator('#v17PresetImportForm select[name="strategy"]').selectOption('rename');await page.locator('#v17PresetImportForm button[type="submit"]').click();await expect(page.locator('#v17PresetStatus')).toContainText('1 xung đột');await expect(page.locator('#v17ApplyPreset')).toBeEnabled();
});
...
test('evidence package blocks tamper through v17 and mobile overview passes Axe',async({page})=>{
  await child(page);await expect(page.locator('#v16EvidenceImportForm')).toBeHidden();const pkg=buildCompatibilityEvidencePackage({compatibilityEvidenceRecords:[{module:'legacy.js',flow:'skills',observed:true,active:false,at:'2026-09-08T01:00:00.000Z'}]});const bad=structuredClone(pkg);bad.records[0].flow='health';
  await page.locator('#v17EvidenceImportForm input[name="file"]').setInputFiles({name:'bad.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(bad))});await page.locator('#v17EvidenceImportForm button[type="submit"]').click();await expect(page.locator('#v17ApplyEvidence')).toBeDisabled();
  await page.locator('#v17EvidenceImportForm input[name="file"]').setInputFiles({name:'ok.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(pkg))});await page.locator('#v17EvidenceImportForm button[type="submit"]').click();await expect(page.locator('#v17ApplyEvidence')).toBeEnabled();
