...
function persist(state,type,details={}){saveState(appendAudit(state,type,details));location.reload();}
...
function domainHistoryPanel(state,child){
  ...
  const records=[
    ...child.learningGoals.map((record)=>({type:'learningGoal',title:record.title,record})),
    ...child.skills.map((record)=>({type:'skill',title:record.name,record})),
    ...child.portfolio.map((record)=>({type:'portfolio',title:record.title,record}))
  ];
...
function saveDomainRecord(form){
  ...
  const fd=new FormData(form),d=Object.fromEntries(fd),domainId=String(fd.get('developmentDomainId')||'');const config=state.settings?.developmentTaxonomy||{};const copy=structuredClone(child);let record=null,type='';
  if(form.id==='learningForm'){
    record=bindDevelopmentDomain({id:makeId('goal'),title:String(d.title||'').trim(),subject:d.subject||'',dueDate:d.dueDate||'',completed:false,createdAt:new Date().toISOString()},domainId,config);copy.learningGoals.push(record);type='learning_goal_created';
  }else if(form.id==='skillForm'){
    const name=String(d.name||'').trim(),old=copy.skills.find((skill)=>String(skill.name).toLowerCase()===name.toLowerCase());
    if(old){old.level=Number(d.level);old.date=todayKey();record=bindDevelopmentDomain(old,domainId,config);Object.assign(old,record);}else{record=bindDevelopmentDomain({id:makeId('skill'),name,level:Number(d.level),date:todayKey()},domainId,config);copy.skills.push(record);}type='skill_saved';
  }else if(form.id==='portfolioForm'){
    record=bindDevelopmentDomain({id:makeId('portfolio'),title:String(d.title||'').trim(),type:d.type||'',date:d.date||todayKey(),note:String(d.note||'').trim()},domainId,config);copy.portfolio.push(record);type='portfolio_created';
  }
  if(!record)return;state.children[idx]=copy;persist(state,type,{childId:child.id,status:record.developmentDomainId?'domain-bound':'legacy-label',taxonomyVersion:record.developmentTaxonomyVersion||null});
}

document.addEventListener('submit',async(event)=>{
  if(['learningForm','skillForm','portfolioForm'].includes(event.target.id)){
    event.preventDefault();event.stopImmediatePropagation();saveDomainRecord(event.target);return;
  }
  if(event.target.id==='v9FamilyIcsForm'){
    event.preventDefault();const ids=new FormData(event.target).getAll('itemIds');if(!ids.length){alert('Hãy chọn ít nhất một mục kế hoạch để xuất lịch.');return;}const state=readState();const ics=familyPlanItemsToIcs(state.settings?.familyPlanItems||[],{itemIds:ids});download(`growup-family-plan-${todayKey()}.ics`,ics,'text/calendar;charset=utf-8');return;
  }
  if(event.target.id==='v9SearchForm'){
    event.preventDefault();const fd=new FormData(event.target);const state=readState();const results=searchSafeDevelopment(state,String(fd.get('query')||''),{childId:String(fd.get('childId')||''),datasets:fd.getAll('datasets'),limit:Number(fd.get('limit'))||20});const target=document.querySelector('#v9SearchResults');if(!target)return;target.innerHTML=results.length?`<div class="v9-list">${results.map((result)=>`<div class="v9-row"><div><strong>${escapeHtml(result.title)}</strong><small>${escapeHtml(DATASET_LABELS[result.dataset]||result.dataset)}${result.subtitle?` · ${escapeHtml(result.subtitle)}`:''}${result.date?` · ${dateLabel(result.date)}`:''}</small></div>${result.childName?`<span class="pill">${escapeHtml(result.childName)}</span>`:''}</div>`).join('')}</div>`:'<div class="empty">Không tìm thấy kết quả trong các dataset an toàn đã chọn.</div>';return;
  }
  if(event.target.id==='v9RecoveryForm'){
    event.preventDefault();const fd=new FormData(event.target),file=fd.get('file'),action=event.submitter?.dataset?.recoveryAction||'inspect',target=document.querySelector('#v9RecoveryResult');if(!(file instanceof File)||!target){return;}const text=await file.text();
    if(action==='inspect'){
      const result=inspectEncryptedBackup(text);target.innerHTML=`<div class="v9-inspection"><strong>${result.supported?'Tương thích':'Chưa tương thích'}</strong><span>Format: ${escapeHtml(result.format||'—')} · KDF: ${escapeHtml(result.kdf||'—')}/${escapeHtml(result.hash||'—')} · ${result.iterations||'—'} vòng · Cipher: ${escapeHtml(result.cipher||'—')}-${result.keyLength||'—'}</span>${result.issues?.length?`<small>${result.issues.map(escapeHtml).join(' · ')}</small>`:''}</div>`;
    }else{
      const passphrase=String(fd.get('passphrase')||'');if(passphrase.length<10){alert('Nhập passphrase backup để chạy recovery drill.');return;}const result=await runRecoveryDrill(text,passphrase);event.target.querySelector('input[name="passphrase"]').value='';target.innerHTML=result.success?`<div class="v9-inspection success"><strong>Recovery drill PASS</strong><span>Schema v${result.schemaVersion||'—'} · ${result.preview?.children||0} hồ sơ trẻ · ${result.preview?.goals||0} mục tiêu · ${result.preview?.healthRecords||0} bản ghi sức khỏe</span><small>Không có dữ liệu nào được restore.</small></div>`:`<div class="v9-inspection error"><strong>Recovery drill FAIL</strong><span>${escapeHtml(result.error||'Không thể kiểm tra.')}</span></div>`;
    }return;
  }
},true);
