...
function safeRowShape(row){if(!row||typeof row!=='object'||Array.isArray(row))return false;const keys=Object.keys(row);if(keys.some((key)=>!ROW_KEYS.has(key)))return false;if(!text(row.id,120)||!['same','different','local-only','file-only'].includes(row.status))return false;for(const key of ['localCompleted','fileCompleted'])if(row[key]!==null&&typeof row[key]!=='boolean')return false;return true;}
...
export function verifyRecoveryReconciliationReportIntegrity(pkg={}){
  const expected=text(pkg?.manifest?.checksum,128),actual=sha256Hex(canonicalStringify(withoutManifest(pkg))),shape=pkg?.format===RECOVERY_RECONCILIATION_REPORT_FORMAT&&Number(pkg?.version)===1&&Array.isArray(pkg?.rows)&&pkg.rows.every(safeRowShape);
  return {valid:Boolean(expected)&&shape&&pkg?.manifest?.format===RECOVERY_RECONCILIATION_REPORT_INTEGRITY_FORMAT&&pkg?.manifest?.algorithm==='SHA-256'&&expected===actual,format:pkg?.manifest?.format||null,algorithm:'SHA-256',expected,actual,rowCount:array(pkg?.rows).length};
}
...
export const RECOVERY_RECONCILIATION_REPORT_INTEGRITY_NOTE='Recovery reconciliation report được ký manifest SHA-256 và verifier chỉ chấp nhận đúng row metadata id/status/date/completed/originDate. Receipt cục bộ chỉ giữ at/format/algorithm/valid-invalid/rowCount, không lưu report nguồn, checksum expected/actual, tên trẻ, reminder title, ICS hoặc API apply/calendar mutation.';
