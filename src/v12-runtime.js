...
function selectionSignature(selection){return JSON.stringify({childIds:[...selection.childIds].sort(),datasets:[...selection.datasets].sort()});}
function renderExportPreview(preview,form,signature){const target=document.querySelector('#v12ExportPreview');if(!target)return;target.innerHTML=`<div class="v12-preview"><strong>${preview.childCount} hồ sơ · ${preview.totalRecords} bản ghi</strong>${preview.datasets.map((row)=>`<div><span>${esc(DATASET_LABELS[row.dataset]||row.dataset)}: ${row.records}</span><small>Trường: ${row.fields.map(esc).join(', ')}</small></div>`).join('')}<small>Health: không · Nutrition: không</small></div>`;form.dataset.previewSignature=signature;form.querySelector('[data-export-action="download"]').disabled=!(preview.childCount&&preview.datasetCount);}
function invalidateExportPreview(form){form.dataset.previewSignature='';const button=form.querySelector('[data-export-action="download"]');if(button)button.disabled=true;const target=document.querySelector('#v12ExportPreview');if(target)target.innerHTML='<div class="empty">Lựa chọn đã thay đổi. Hãy xem preview lại trước khi tải.</div>';}

document.addEventListener('submit',(event)=>{
  if(event.target.id==='v12ExportForm'){
    event.preventDefault();const state=readState(),selection=exportSelection(event.target),signature=selectionSignature(selection),preview=previewSafeExport(state,selection),action=event.submitter?.dataset.exportAction;
    if(action==='preview'){renderExportPreview(preview,event.target,signature);return;}
    if(action==='download'){
      if(event.target.dataset.previewSignature!==signature){renderExportPreview(preview,event.target,signature);alert('Lựa chọn đã thay đổi hoặc chưa được preview. Hãy kiểm tra preview rồi bấm tải lại.');return;}
      try{const payload=buildSafeExportPackage(state,selection);download(`growup-safe-export-${Date.now()}.json`,JSON.stringify(payload,null,2));saveAudit(state,'safe_export_wizard_downloaded',{children:preview.childCount,datasets:preview.datasetCount,records:preview.totalRecords});}catch(error){alert(error.message);}return;
    }
  }
...
document.addEventListener('change',(event)=>{if(event.target.id==='v12WorkloadMonth')renderWorkloadCalendar(readState(),event.target.value);if(event.target.closest('#v12ExportForm'))invalidateExportPreview(event.target.closest('#v12ExportForm'));});
