...
- Lượt 116: Receipt import-history package verification receipt management. **Completed in stable v1.9.0 with bounded metadata-only verify history/filter/export/confirmed-clear; source history package is never restored.**
- Lượt 117: Preset audit review filters/comparison summaries. **Completed in stable v1.9.0 as read-only neutral review over strategy/status/conflict/source→resolved metadata; no preset mutation or scoring semantics.**
- Lượt 118: Saved Search receipt import-history portability integrity. **Completed in stable v1.9.0 with SHA-256 export/verify/review; criteria/result/snippet are excluded and undo remains local to L113 managed state.**
- Lượt 119: Recovery verification-receipt package integrity/verification. **Completed in stable v1.9.0 with SHA-256 plus bounded metadata-only verification history; unsigned v1.8 export is retired while filter/confirmed-clear remains, with no report/calendar mutation.**
- Lượt 120: Compatibility freshness review export/integrity + explicit local Axe recheck. **Completed in stable v1.9.0 with signed review, pending recheck requests and PASS recording only after `source=actual-local-axe`; always `retirementAllowed:false`/`actionsApplied:false`. PR #24 exact final head `3c04e013c7f14ca35498399da9eb16502ce29f09`, final PR CI run `34223017894` PASS verify + Chromium/Axe; merged main `7740f7ec90992811fd0ea66b0654086dcc25d116`, post-merge CI run `34223533738` PASS both source gates. Pages run `34223533614` fails separately at `Configure Pages`; Upload/Deploy/Verify are skipped by L31/issue #2.**
