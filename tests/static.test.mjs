...
test('document exposes Vietnamese language, viewport and v7 runtime assets', async () => {
  const html = await read('index.html');
  assert.match(html, /<html lang="vi">/);
  assert.match(html, /name="viewport"/);
  for (const asset of ['enhancements.css','enhancements.js','v4.css','v4.js','v5.css','v5.js','v6.css','v6.js','v7.css','v7.js','a11y.js']) {
    assert.ok(html.includes(asset), `index should load ${asset}`);
  }
});

test('privacy restore export retention and accessibility controls expose safe actions', async () => {
...
test('service worker caches every v7 runtime module', async () => {
  const sw = await read('sw.js');
  for (const asset of ['enhancements.js','v4.js','v5.js','v6.js','v7.js','a11y.js','schema.js','analytics.js','calendar.js','attachments.js','integrations.js','privacy.js','roles.js','reminders.js','backup.js','templates.js','report.js','timeline.js','evidence.js','family-policy.js','export.js','evidence-repair.js','archive.js','timeline-filter.js','consistency.js','retention.js','yearly-summary.js','print-report.js']) {
    assert.ok(sw.includes(asset), `service worker should cache ${asset}`);
  }
  assert.match(sw, /growup-mychildren-v7/);
});
